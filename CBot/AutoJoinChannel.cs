...
namespace CBot {
	...
	public struct AutoJoinChannel {
		...
		/// <exception cref="System.ArgumentException">channel contains a delimiter (space, null, newline or comma).</exception>
		...
		public AutoJoinChannel(string channel, string key) {
			if (channel != null && channel.IndexOfAny(new char[] { ' ', '\0', '\n', ',' }) != -1)
				throw new ArgumentException("Invalid characters in the channel name.", "channel");

			if (key != null && key.IndexOfAny(new char[] { ' ', '\0', '\n', ',' }) != -1)
				throw new ArgumentException("Invalid characters in the channel key.", "key");
